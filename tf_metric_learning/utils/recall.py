...
class AnnoyEvaluatorCallback(AnnoyDataIndex):
    ...
    """
    def __init__(self, model, save_dir, data_store, data_search, emb_size=256, metric="euclidean", freq=1, batch_size=None, normalize_eb=True, progress=True, **kwargs):
        super().__init__(emb_size, metric, data_store["labels"], save_dir=save_dir, progress=progress)
    ...
    def on_epoch_begin(self, epoch, logs=None):
        ...
        embeddings_store = self.base_model.predict(self.data_store["images"], batch_size=self.batch_size)
        embeddings_search = self.base_model.predict(self.data_search["images"], batch_size=self.batch_size)

        if self.normalize_eb:
            embeddings_store = tf.nn.l2_normalize(embeddings_store, axis=1)
            embeddings_search = tf.nn.l2_normalize(embeddings_search, axis=1)

        self.reindex(embeddings_store)

        results = []
        for i, embedding in tqdm(enumerate(embeddings_search), ncols=100, total=len(embeddings_store), disable=not self.progress, desc="Search/Recall"):
